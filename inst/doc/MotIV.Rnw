...
\author{Eloi Mercier\footnote{eloi.mercier@ircm.qc.ca} and Raphael Gottardo\footnote{raphael.gottardo@ircm.qc.ca}}
...
\verb@Motiv@ is based on the C++ functions of the STAMP algorithm \cite{STAMP} and it also use a modified version of the SeqLogo package \cite{SeqLogo}. Please cite the following paper if you use \verb@MotIV@ for publication :
\begin{center}
...
<<Read input PWM>>=
sample.motifs <- readPWMfile(paste(path,"sample.motifs",sep=""))
@

\subsection{Analysis}
<<start the analysis, eval=T>>=
example.jaspar <- motifMatch(inputPWM=sample.motifs,align="SWU",cc="PCC",database=jaspar,DBscores=jaspar.scores,top=5)
@
...
<<Viewresults, fig=F, eval=T>>=
EL.filter <- setFilter(name="EL")
SP.filter <- setFilter(name="SP")
EL.SP.filter <- EL.filter | SP.filter
example.filter <- filter(example.jaspar,EL.SP.filter, exact=F)
...
<<write scores, eval=F>>=
writeDBScores(jaspar.scores,paste(path,"jaspar_PCC_SWU.scores",sep=""))
@
...
motifs.gadem <- readGademPWMFile(paste(path,"observedPWMs.txt",sep=""))
@
...
motifs.sample <- readPWMfile(paste(path,"sample.motifs",sep=""))
@
...
plot(stat1.filter.combine ,ncol=2,top=5, rev=FALSE, main="Logo", bysim=TRUE)
@
\end{center}
...
stat1.alignment <- viewAlignments(stat1.filter.combine )
print(stat1.alignment[[1]] )
@
...
plot(stat1.filter.combine ,gadem,ncol=2, type="distribution", correction=TRUE, group=FALSE, bysim=TRUE, strand=FALSE, sort=TRUE, main="Distribution of STAT1")
@
\end{center}
...
plot(stat1.filter.combine ,gadem,type="distance", correction=TRUE, group=TRUE, bysim=TRUE, main="Distance between STAT1 and AP-1", strand=FALSE, method=3, xlim=c(-100,100), bw=8)
@
\end{center}
...
A \verb@rangedData@ is an object created by the \verb@IRanges@ library \cite{IRanges}. It's a really flexible object.
...
ap1.rd <- exportAsRangedData(stat1.filter.combine["STATx"], gadem)
@

\subsection{Modules}

If you are interesting by modules discovery, you should use the \verb@calculateModules@ function. This function use two \verb@RangedData@ objects containing the motifs coordinates and computes the distances between them. Then, it returns the modules composed of two close motifs.
...
<<motiv save, eval=T>>=
save(stat1.filter.combine, file="stat1_analysis.rda")
@
...
<<bedfile, eval=F>>=
library(rtracklayer)
export(stat1.rd, file="STAT1.bed")
@
...
<<viewMotifs, eval=T>>=
viewMotifs(stat1.filter.combine, n=5)
@
...
<<names, eval=T>>=
names(stat1.filter.combine)
@

\subsection{similar}

The \verb@similar@ function shows the names of the similar motifs in a \verb@motiv@ object.

<<similar, eval=T>>=
similar(stat1.filter.combine)
@
...
<<select, eval=T>>=
stat1.selected <- stat1.filter.combine["STATx"]
other.selected <- stat1.filter.combine["STATx", drop=T]
@

Combine with other functions, it can be really useful. To know how many motifs STATx you got, try by instance :

<<select number, eval=T>>=
stat1.names <- names(stat1.filter.combine["STATx"])
sum(length(gadem[stat1.names]))
@
