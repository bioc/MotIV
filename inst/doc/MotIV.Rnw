...
<<Load the database>>=
jaspar <- readPWMfile(paste(path,"/extdata/jasparDB2009.txt",sep=""))
@

\subsection{Load database scores}
<<Load database scores>>=
jaspar.scores <- readDBScores(paste(path,"/extdata/jaspar2009_PCC_SWU.scores",sep=""))
@

\subsection{Read input PWM}
<<Read input PWM>>=
sample.motifs <- readPWMfile(paste(path,"/extdata/sample.motifs",sep=""))
@
...
load(paste(path,"/data/gadem_STAT1_MACS.rda",sep=""))
...
motifs.sample <- readPWMfile(paste(path,"/extdata/sample.motifs",sep=""))
@
...
plot(stat1.filter.combine ,gadem,type="distance", correction=TRUE, group=TRUE, bysim=TRUE, main="Distance between STAT1 and AP-1", strand=FALSE, xlim=c(-100,100), darg=list(bw=8))
@
\end{center}
...
<<similar, eval=T>>=
similarity(stat1.filter.combine)
@
