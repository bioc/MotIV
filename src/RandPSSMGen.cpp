...
SEXP RandPSSMGen::RunGenerator()
{
	...
	SEXP listRandPWM, randPWM, names;
	...
	for(z=0; z<numRandomMats; z++) {
		...
		for(q=0; q<curr_len; q++){
			for(w=0; w<B; w++){
				...
				DOUBLE_DATA(randPWM)[PWMcount]=newPSSM->n[q][w] ;				
				PWMcount++;
			}
		}
		SET_STRING_ELT(names, z, mkChar("rand"));
		SET_VECTOR_ELT(listRandPWM,z,randPWM);
		UNPROTECT(1);
	}
	SET_NAMES(listRandPWM, names);	
	UNPROTECT(1); // names	
	Rprintf("\tRandom Matrix generation completed :\n");
	Rprintf("\t\tNew Zeros: %lf\n", new_zeros/new_total);
