...
{	
  ...
  for (j in 1:length(gadem@motifList))
  {
    ...
    if (gadem@motifList[[j]]@name %in% motivnames)
    {
      ...
      if (j>1 && any(similar(pos) %in% motiv@bestMatch[[current.motiv]]@similar) && group)
      {
        whichSimilarMotif=which(similar(pos)%in% motiv@bestMatch[[current.motiv]]@similar)
        pos[[whichSimilarMotif]] <- new("position", motifName=c(pos[[whichSimilarMotif]]@motifName, gadem@motifList[[j]]@name), positionVector=rbind(pos[[whichSimilarMotif]]@positionVector, vector.pos), pwm=c(pos[[whichSimilarMotif]]@pwm, list.pwm), similarMotif=pos[[whichSimilarMotif]]@similarMotif, length=gadem@motifList[[1]]@alignList[[1]]@end-gadem@motifList[[1]]@alignList[[1]]@start )
      } else   {
        pos[[pos.count]] <- new("position", motifName=gadem@motifList[[j]]@name, positionVector=vector.pos, pwm=list.pwm, similarMotif=motiv@bestMatch[[current.motiv]]@similar, length=gadem@motifList[[1]]@alignList[[1]]@end-gadem@motifList[[1]]@alignList[[1]]@start )
