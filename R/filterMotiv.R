...
filterMotiv <- function (x, f, exact, verbose)
{
  ...
  {
    selectedMatch <- c(selectedMatch, new("matches", name=x@bestMatch[[l]]@name, aligns=x@bestMatch[[l]]@aligns, similar=x@bestMatch[[l]]@similar, valid=validMotifs[countSelectedMotifs])	)  	
    ...
    if (length(selectedMotifs) >0)
    {
      ...
      for (l in selectedMotifs)
      {
        ...
        if (!is.null(name))
        {
          ...
          x@bestMatch[[l]]@similar <- name[filterNumber]
        }
        else
        {
          combinedMotifs <- c(combinedMotifs, originalNames[l])
          x@bestMatch[[l]]@similar <- paste("similar", filterNumber, sep="")
        }
      }
      if(verbose)
...
splitMotiv <- function(x, f, drop, exact, verbose)
{
  ...
  if (length(motivDrop@input)!=0)
  {
    ...
    for (l in 1:length(x@bestMatch))
    {
      ...
      {
        bestMatch <- c(bestMatch, new("matches", name=x@bestMatch[[l]]@name, aligns=x@bestMatch[[l]]@aligns, similar=x@bestMatch[[l]]@similar, valid=x@bestMatch[[l]]@valid)	) 
      }
    }
